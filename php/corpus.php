...
function update_corpus($mysqli, $id) {
  ...
  INNER JOIN commitment AS e ON e.id=pep.id AND e.accepted=1
  WHERE pep.`key`=pc.`key` AND e.publication=pp.signature
  EOT;
  $count = <<<EOT
  ...
  WHERE commitment.`type`='endorse' OR (commitment.`type`='report' AND EXISTS($accepted))
  EOT;
