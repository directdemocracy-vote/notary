...
if (!$results->secret) {  # public referendum
  # count participation
  $query = "SELECT COUNT(citizen) AS participation FROM ballot INNER JOIN corpus ON corpus.citizen=ballot.`key` AND corpus.referendum=ballot.referendum WHERE ballot.referendum=$referendum_id";
  $result = $mysqli->query($query) or error($mysqli->error);
